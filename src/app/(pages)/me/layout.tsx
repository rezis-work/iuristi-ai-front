import React from "react";
import Wrapper from "@/src/components/shared/wrapper";
import { AccountSidebar } from "@/src/features/user-account/components/account-sidebar";
import { LawyerProfileMenu } from "../../../features/user-account/components/lawyer-profile-menu";
import { AuthGuard } from "@/src/components/auth/AuthGuard";

function Layout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <AuthGuard>
    <div className="bg-[#111111] text-neutral-100 overflow-x-hidden">
      <Wrapper className="mx-auto px-4 lg:px-6 2xl:px-28">
        <div className="flex flex-col md:grid md:grid-cols-8 gap-6 sm:gap-6 md:gap-4 lg:gap-6 py-6 sm:py-8 2xl:flex-row 2xl:py-10">
          <div className="col-span-8 xl:col-span-2">
            <AccountSidebar />
          </div>
          <div className="w-full min-w-0 col-span-8 xl:col-span-6">
            <LawyerProfileMenu />
            {/* Content */}
            <div className="w-full min-w-0 rounded-sm bg-[#181818] px-4 py-5 sm:px-5 sm:py-6 2xl:px-6 flex-1 overflow-x-hidden">
              {children}
            </div>
          </div>
        </div>
      </Wrapper>
    </div>
    </AuthGuard>
  );
}

export default Layout;
